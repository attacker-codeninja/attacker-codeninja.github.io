...
layout: base
title: Bug Bounty
category_name: Bug Bounty
permalink: /category/bug-bounty/
---
...
<div style="display: flex; flex-direction: column; align-items: flex-start; gap: 18px; margin-bottom: 50px;">
  ...
  <div style="font-family: var(--mono); font-weight: 700; font-size: clamp(34px, 6vw, 64px); line-height: 1.05; letter-spacing: -0.02em; color: var(--red);">
    Bug Bounty
  </div>
</div>
...
<div class="grid cols-2" id="post-grid" style="margin-top: 40px; display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 24px;">
  {% for post in site.categories["Bug Bounty"] %}
    <a href="{{ post.url | relative_url }}" class="post-list-item cat-card" style="display: none; background: rgba(229, 72, 77, 0.05); border: 1px solid rgba(229, 72, 77, 0.3); padding: 30px; align-items: flex-start; text-align: left; height: 100%;">
      <h2 style="color: var(--red); font-size: 20px; line-height: 1.4; margin-bottom: 12px;">{{ post.title }}</h2>
